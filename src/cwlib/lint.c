 ...
 * Copyright (c) 2002-2026, T. L. Turocy, Chadwick Baseball Bureau (ted.turocy@gmail.com)
 ...
 */
static int cw_game_lint_starters(CWGame *game)
{
  int ok = 1;
  CWAppearance *app;
 ...
 */
static int cw_game_lint_state(CWGameIterator *gameiter)
{
  int ok = 1;
  int src, dest;
  ...
  if (gameiter->event_data->dp_flag && cw_event_outs_on_play(gameiter->event_data) < 2) {
  ...
  if (gameiter->event_data->tp_flag && cw_event_outs_on_play(gameiter->event_data) < 3) {
    fprintf(stderr, "Play-by-play error in game %s at event %d:\n", gameiter->game->game_id,
            gameiter->state->event_count + 1);
  ...
  for (dest = 1; dest <= 3; dest++) {
    ...
    for (src = 0; src < dest; src++) {
      ...
      if (srcAdv >= destAdv && !cw_event_runner_put_out(gameiter->event_data, dest) &&
          destAdv < 4 && gameiter->state->outs + cw_event_outs_on_play(gameiter->event_data) < 3) {
        fprintf(stderr, "Play-by-play error in game %s at event %d:\n", gameiter->game->game_id,
                gameiter->state->event_count + 1);
        fprintf(stderr, "Runner on %d overtaken by runner on %d (event \"%s\", %s batting)\n",
                dest, src, gameiter->event->event_text, gameiter->event->batter);
        ok = 0;
      }
    }
  }
